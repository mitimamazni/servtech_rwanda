        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
          ...
          {step === 0 && (
            <div className="space-y-4">
              ...
              <p className="text-sm text-gray-500">Enter the client's national ID number to check the registry and auto-fill their details.</p>
              <div>
              ...
              {verificationStatus === 'not_found' && (
                <div className="flex items-center gap-2 text-amber-600 text-sm bg-amber-50 border border-amber-200 rounded-lg px-4 py-3">
                  <AlertCircle size={16} /> ID not found - registration will be pending
                </div>
              )}
              {verificationStatus === 'underage' && (
                <div className="flex items-center gap-2 text-red-600 text-sm bg-red-50 border border-red-200 rounded-lg px-4 py-3">
                  <AlertCircle size={16} /> Registration denied - client is under 18
                </div>
              )}
              <button onClick={handleVerify} disabled={verifying || !form.id_number}
                ...
                {verifying ? <><Loader size={14} className="animate-spin" /> Checking registry...</> : 'Look up ID'}
              </button>
              <button onClick={() => setStep(1)} className="w-full text-sm text-gray-400 hover:text-gray-600 py-2">
          ...
          {step === 1 && (
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                ...
                {verificationStatus === 'verified' && (
                  <span className="text-xs text-green-600 flex items-center gap-1"><CheckCircle size={12} /> Auto-filled</span>
                )}
              </div>
              {[{label:'First Name',field:'first_name',type:'text',req:true},{label:'Last Name',field:'last_name',type:'text',req:true},{label:'Date of Birth',field:'date_of_birth',type:'date',req:false}].map(({label,field,type,req}) => (
                <div key={field}>
                  <label className="block text-sm font-medium text-gray-700 mb-1">{label} {req && <span className="text-red-400">*</span>}</label>
          ...
          {step === 3 && (
            <div className="space-y-4">
              ...
              <div className="bg-gray-50 rounded-xl p-4 space-y-3">
                ...
                {[['Date of Birth',form.date_of_birth],['Igitsina',form.gender],['Phone',form.phone],['District',form.district],['Verification',verificationStatus==='verified'?'Verified':'Pending review']].map(([label,value]) => (
              ...
              <div className="bg-blue-50 border border-blue-200 rounded-lg px-4 py-3 text-sm text-blue-700">
                By submitting, you confirm you have verified the client's identity in person.
              </div>
            </div>
          )}
          ...
          <div className="flex justify-between mt-6">
            ...
            {step < STEPS.length-1 ? (
              <button onClick={() => setStep(s=>s+1)} disabled={verificationStatus==='underage'}
                className="bg-primary-600 hover:bg-primary-700 text-white text-sm px-5 py-2.5 rounded-lg disabled:opacity-60 transition-colors">
                Continue
              </button>
            ) : (
