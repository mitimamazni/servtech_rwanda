...
export default function AgentDashboard() {
  ...
  useEffect(() => {
    axios.get('/stats').then(r => setStats(r.data)).catch(() => {});
  }, []);
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-6xl mx-auto px-6 py-8 space-y-8">
        <div>
          <h2 className="text-xl font-semibold text-gray-800">My Registrations</h2>
          <p className="text-sm text-gray-500 mt-1">Clients you have registered</p>
        </div>

        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {stats ? (
            <>
              <StatCard label="My Clients"       value={stats.total}    icon={Users}        color="bg-primary-600" />
              <StatCard label="Verified"          value={stats.verified} icon={UserCheck}    color="bg-green-500" />
              <StatCard label="Pending"           value={stats.pending}  icon={UserX}        color="bg-amber-500" />
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm">
          <div className="p-5 border-b border-gray-100 flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between">
            <h3 className="font-medium text-gray-800">Client List <span className="text-sm text-gray-400 font-normal">{total} total</span></h3>
            <div className="flex gap-2 w-full sm:w-auto">
              <form onSubmit={e => { e.preventDefault(); setSearch(searchInput); setPage(1); }} className="flex gap-2 flex-1 sm:flex-none">
                ...
                <button type="submit" className="px-3 py-2 text-sm bg-gray-800 text-white rounded-lg">Go</button>
              </form>
          ...
          {loading ? (
            ...
            <div className="text-center py-16 text-gray-400">
              ...
              <button onClick={() => navigate('/agent/register')} className="mt-3 text-primary-600 text-sm hover:underline">
