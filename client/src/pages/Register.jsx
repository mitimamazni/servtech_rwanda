...
export default function Register() {
  ...
  const handleImageChange = async (e) => {
    ...
    try {
      ...
      if (extracted.confidence > 70) {
        toast.success(`ID scanned successfully (${extracted.confidence.toFixed(0)}% confidence)`);
      } else {
        toast('Scan complete - please review and correct the fields below', { icon: '⚠️' });
      }
    } catch {
      toast.error('OCR failed - please fill in the details manually');
    } finally {
      setOcrLoading(false);
    }
  };
  ...
  const handleVerify = async () => {
    if (!form.id_number) {
      toast.error('Please enter or scan an ID number first');
      return;
    }
    setVerifying(true);
    ...
    try {
      ...
      toast('ID not found in registry - registration will be marked as pending', { icon: '⚠️' });
    } finally {
      setVerifying(false);
    }
  };
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
          ...
          {step === 0 && (
            <div className="space-y-4">
              ...
              <p className="text-sm text-gray-500">
                Take a clear photo of the client's national ID card. The system will automatically extract their details.
              </p>
              ...
              >
                {imagePreview ? (
                  <img src={imagePreview} alt="ID preview" className="max-h-48 mx-auto rounded-lg object-cover" />
                ) : (
              ...
              <p className="text-xs text-gray-400 text-center">
                No photo?{' '}
                <button onClick={() => setStep(1)} className="text-primary-600 underline">
          ...
          {step === 1 && (
            <div className="space-y-4">
              ...
              <p className="text-sm text-gray-500">Check the extracted details below. Correct any errors before verifying.</p>
              ...
              <div>
                ...
                <div className="flex gap-2">
                  ...
                  >
                    {verifying ? <Loader size={14} className="animate-spin" /> : null}
                    Verify
                  </button>
                </div>
                {verificationStatus === 'verified' && (
                  <p className="text-green-600 text-xs mt-1 flex items-center gap-1">
                    <CheckCircle size={12} /> Verified against national registry
                  </p>
                )}
                {verificationStatus === 'not_found' && (
                  <p className="text-amber-600 text-xs mt-1 flex items-center gap-1">
                    <AlertCircle size={12} /> Not found - will be marked pending
                  </p>
                )}
              </div>
          ...
          {step === 2 && (
            <div className="space-y-4">
              ...
              <p className="text-sm text-gray-500">Enter the client's phone number and district.</p>
          ...
          {step === 3 && (
            <div className="space-y-4">
              ...
              <p className="text-sm text-gray-500">Review all details before submitting.</p>
              ...
              <div className="bg-gray-50 rounded-xl p-4 space-y-3">
                <div className="flex items-center gap-3 pb-3 border-b border-gray-200">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-500">{form.id_number}</p>
                  </div>
                  {verificationStatus === 'verified' && (
                ...
                {[
                  ...
                  ['Status', verificationStatus === 'verified' ? 'Will be verified' : 'Will be pending'],
                ].map(([label, value]) => (
