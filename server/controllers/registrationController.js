...
exports.registerClient = async (req, res) => {
  ...
  try {
    ...
    if (existing.rows.length > 0) {
      return res.status(400).json({ message: 'Client with this ID is already registered' });
    }

    const idCheck = await pool.query(
      'SELECT id FROM id_records WHERE id_number = $1 AND valid = true',
      [id_number]
    );
    ...
    const result = await pool.query(
      `INSERT INTO clients
        (id_number, first_name, last_name, date_of_birth, gender, phone, district, status, registered_by)
...
exports.getClients = async (req, res) => {
  ...
  try {
    ...
    if (search) {
      ...
      countQuery += ` AND (first_name ILIKE $${countParams.length} OR last_name ILIKE $${countParams.length} OR id_number ILIKE $${countParams.length})`;
    }
    if (status) {
...
exports.getStats = async (req, res) => {
  try {
    ...
    const verified = await pool.query("SELECT COUNT(*) FROM clients WHERE status = 'verified'");
    const pending  = await pool.query("SELECT COUNT(*) FROM clients WHERE status = 'pending'");
    const today    = await pool.query("SELECT COUNT(*) FROM clients WHERE created_at::date = CURRENT_DATE");
