...
const SAFE_CLIENT_COLUMNS = `
  ...
  wallet_balance, self_exclusion_until, created_at, kyc_submitted_at,
  ...
  try {
    const clientResult = await pool.query(
      `SELECT c.id, c.user_id, c.id_number, c.first_name, c.last_name, c.date_of_birth,
              ...
              c.elderly_assisted, c.registered_by, c.created_at, c.wallet_balance, c.self_exclusion_until, u.email
       FROM clients c
...
exports.getClientActivity = async (req, res) => {
  ...
  try {
    const clientResult = await pool.query(
      `SELECT c.id, c.user_id, c.id_number, c.first_name, c.last_name, c.date_of_birth,
              c.gender, c.phone, c.district, c.status, c.rejection_reason, c.is_active,
              c.elderly_assisted, c.registered_by, c.created_at, c.kyc_submitted_at, c.wallet_balance, c.self_exclusion_until,
